    ...
    def make_api_request(self, content: str, max_retries: int = 3) -> Optional[str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.request_method == 'siliconflow':
                    # 保持原有的siliconflow请求方式
                    import requests
                    payload = {
                        "model": "qwen3-max-preview/Pro/deepseek-ai/DeepSeek-R1",
                ...
                elif self.request_method == 'deepseek':
                    # 保持原有的deepseek请求方式
                ...
                elif self.request_method == 'qwen3-max-preview':
                    # 使用OpenAI客户端调用千问API
                    client = OpenAI(
                    ...
                    response = client.chat.completions.create(
                        model="qwen3-max-preview",  # 可根据需要调整模型版本
                        messages=[{"role": "user", "content": content}],
                        temperature=0.3,
                        stream=False
                    )
